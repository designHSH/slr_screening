...
def _askr_json_schema(max_askr_items: int) -> Dict[str, Any]:
    ...
    conf_enum = sorted(list(ALLOWED_CONF))
    ...
    return {
        ...
        "schema": {
            ...
            "required": ["workload", "askr"],
            "properties": {
                "workload": {
                    "type": "object",
                    "additionalProperties": False,
                    "required": ["primary_workload", "secondary_workloads", "workload_summary", "workload_quote"],
                    "properties": {
                        "primary_workload": {
                            "anyOf": [
                        ...
                        "secondary_workloads": {
                            "type": "array",
                            "items": {"type": "string", "enum": wl_enum}
                        },
                        "workload_summary": {"type": ["string", "null"]},
                        "workload_quote": {"type": ["string", "null"]}
                    }
                },
                "askr": {
                    "type": "array",
                    ...
                    "items": {
                        ...
                        "properties": {
                            ...
                            "linked_workload": {"type": "string", "enum": wl_enum},
                            "askr_summary": {"type": "string"},
                            "verbatim_quote": {"type": "string"},
                            "gap_signal": {"type": "boolean"},
                            "short_reasoning": {"type": "string"},
                            "confidence_level": {"type": "string", "enum": conf_enum}
                        }
                    }
                }
            }
...
def process_one_unit(
    *,
    ...
    askr_trimmed_meta: Optional[Dict[str, Any]] = None
    ...
    while True:
        ...
        if prompt.config.trace.save_responses:
            ...
            save_trace_response(log_paths, unit["unit_id"], attempts, json.dumps(response_log, ensure_ascii=False, indent=2))
            ...
            if prompt.config.trace.save_errors:
                ...
                save_trace_error(
                    ...
                    (
                        ...
                        f"REPAIRED HEAD:\n{rep_excerpt_head}\n\nREPAIRED TAIL:\n{rep_excerpt_tail}\n\n"
                        f"RAW FULL:\n{last_raw}\n\nREPAIRED FULL:\n{last_repaired}\n\nREQUEST:\n{request_dump}\n"
                        f"RAW_RESPONSE_OBJ:\n{json.dumps(resp.raw_response, ensure_ascii=False, indent=2) if resp.raw_response else 'null'}"
                    )
                )
            break
        ...
        # Success (optionally trim askr if over cap)
        parsed_obj = obj
        askr_items = parsed_obj.get("askr", []) if isinstance(parsed_obj, dict) else []
        if isinstance(askr_items, list) and len(askr_items) > prompt.config.max_askr_items:
            # Trim only if beyond cap; log in response trace for visibility.
            parsed_obj["askr"] = askr_items[:prompt.config.max_askr_items]
            askr_trimmed_meta = {
                "original_count": len(askr_items),
            ...
            if prompt.config.trace.save_errors:
                save_trace_error(
                    ...
                    unit["unit_id"],
                    attempts,
                    f"ASKR TRIMMED: original={len(askr_items)} kept={prompt.config.max_askr_items}"
                )
