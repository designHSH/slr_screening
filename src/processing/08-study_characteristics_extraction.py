...
# Config (as requested)
# =========================
INPUT_PDF_FOLDER = r"data\all_included_title_abstract_paper_cleaning\included_fulltext\02-b2"
OUTPUT_CSV_FILE = r"data\all_included_title_abstract_paper_cleaning\included_fulltext\01_study_characteristics\02_study_characteristics_b02.csv"
PROMPT_YAML_FILE = r"prompt\processing\study_characteristics_extraction_full_text.yaml"
...
class GeminiStudyCharacteristicsRunner:
    def __init__(
        ...
        thinking_cfg = google_cfg.get("thinking_config", {}) if isinstance(google_cfg.get("thinking_config", {}), dict) else {}

        self.client = genai.Client(api_key=api_key)
        self.model_id = config.get("model", model)
        self.temperature = config.get("temperature", temperature)
        self.response_mime_type = config.get("response_mime_type", response_mime_type)
        self.thinking_level = (
            config.get("THINKING_LEVEL")
            or config.get("thinking_level")
            or thinking_cfg.get("thinking_level")
        )
