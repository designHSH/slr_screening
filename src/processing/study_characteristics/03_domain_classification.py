...
# - sends domain_value and domain_quote to Gemini
...
def build_user_prompt(template: str, domain_value: str, domain_quote: str) -> str:
    has_placeholders = ("{domain_value}" in template) or ("{domain_quote}" in template)
    if has_placeholders:
        try:
    ...
    return (
        template.rstrip()
        + "\n\n"
        + f"domain_value: {domain_value}\n"
        + f"domain_quote: {domain_quote}"
    )
...
def classify_domain(
    ...
    return {
        "domain_clean_main": normalize_empty(parsed.get("domain_clean_main")),
        "domain_clean_subdomain": normalize_empty(parsed.get("domain_clean_subdomain")),
        "manual_review_needed": normalize_empty(parsed.get("manual_review_needed")),
        "coding_note": normalize_empty(parsed.get("coding_note")),
        "domain_llm_raw_response": raw_text,
    }


def setup_logging(log_file: Path) -> logging.Logger:
    logger = logging.getLogger("domain_classification")
    if logger.handlers:
        return logger
...
def main() -> None:
    ...
    print("Domain classification script using Gemini")
    print("-" * 50)
    ...
    logger.info("Domain classification script starting.")
    ...
    required_columns = ["paper_key", "domain_value", "domain_quote"]
    missing = [col for col in required_columns if col not in df.columns]
    if missing:
        raise ValueError(f"Missing required Excel column(s): {', '.join(missing)}")

    # Resume mode: if output exists, skip rows that already have domain_clean_main
    ...
    if output_file.exists():
        ...
        if "paper_key" in out_df.columns and "domain_clean_main" in out_df.columns:
            ...
            for _, r in out_df.iterrows():
                ...
                if not pk:
                    continue
                existing_map[pk] = {
                    "domain_clean_main": normalize_empty(r.get("domain_clean_main", "")),
                    "domain_clean_subdomain": normalize_empty(r.get("domain_clean_subdomain", "")),
                    "manual_review_needed": normalize_empty(r.get("manual_review_needed", "")),
                    "coding_note": normalize_empty(r.get("coding_note", "")),
                    "domain_llm_raw_response": normalize_empty(r.get("domain_llm_raw_response", "")),
                }
        else:
    ...
    try:
        for idx, row in df.iterrows():
            ...
            domain_value = normalize_empty(row.get("domain_value", ""))
            domain_quote = normalize_empty(row.get("domain_quote", ""))

            logger.info("Processing row %d/%d | paper_key=%s", idx + 1, total_rows, paper_key)
            logger.info("Inputs | paper_key=%s | domain_value=%s | domain_quote=%s", paper_key, domain_value, domain_quote)

            try:
                existing = existing_map.get(paper_key)
                if existing and normalize_empty(existing.get("domain_clean_main")):
                    skipped_count += 1
                    logger.info(
                    ...
                    result = existing
                elif not domain_value:
                    skipped_count += 1
                    logger.warning("Skipped row %d | paper_key=%s | reason=empty domain_value", idx + 1, paper_key)
                    result = {
                        "domain_clean_main": "",
                        "domain_clean_subdomain": "",
                        "manual_review_needed": "Yes",
                        "coding_note": "domain_value is empty.",
                        "domain_llm_raw_response": "",
                    }
                else:
